...
public class CMLIN {
    ...
    private Line coupledMicrostripAna(Line CMLINLine, int do_loss) {
        ...
        if (t > 0.0) {
            ...
            deltau1 = (T / Constant.Pi) * Math.log(1.0 + 4.0 * Math.exp(1.0) / (T * Math.pow(
                    Math.cosh(Math.sqrt(6.517 * u))
   ...
   */
        ...
        if (er > 1.0) {
            CMLINLine.setLossLenEven((Constant.Pi * f * Math.sqrt(EFE0) / Constant.LIGHTSPEED) * (er / EFE0) * ((EFE0 - 1.0) / (er - 1.0)) * tand);
            CMLINLine.setLossLenOdd((Constant.Pi * f * Math.sqrt(EFO0) / Constant.LIGHTSPEED) * (er / EFO0) * ((EFO0 - 1.0) / (er - 1.0)) * tand);
        } else {
            // XXX verify this one
   ...
   */
        ...
        CMLINLine.setLossEven(l * CMLINLine.getLossLenEven());
